...
//
    ...
    private void reloadNextTrigger() {
        ...
        if (size > GlobalConstant.NEXT_TRIGGER_MAX_COUNT) {
            log.warn("[TaskInstance] 超过接下来{}秒内需要触发的触发器列表最大值：{}，当前值：{} | instanceName={}", nextTime, GlobalConstant.NEXT_TRIGGER_MAX_COUNT, size, this.getInstanceName());
        }
        ...
        for (final JobTrigger jobTrigger : nextJobTriggerList) {
            ...
            try {
                Future<?> future = schedulerWorker.submit(() -> {
                    final JobTriggerLog jobTriggerLog = newJobTriggerLog(dbNow, jobTrigger);
     ...
     * 执行定时任务逻辑
     */
    private void executeJob(final Date dbNow, final Job job, final JobLog jobLog) {
        final Scheduler scheduler = taskContext.getCurrentScheduler();
        try {
